...
describe(`Zendesk Service`, () => {
  ...
  describe('createSupportDeskTicket', () => {
    it('should create support desk ticket with an organisation and return its ID', done => {
...
`,
      ...
      zendeskService.createSupportTicket(contactHelpdeskForm, username, organisation).then(ticketId => {
        expect(ticketId).toBe(1234)
        done()
      })
    })

    it('should create support desk ticket without an organisation and return its ID', done => {
      const contactHelpdeskForm: ContactHelpdeskForm = {
        pageId: 'review-recipients',
        problemDetail: 'I cant add a recipient',
        name: 'Mrs Legal Sender User',
...
`,
      ...
      zendeskService.createSupportTicket(contactHelpdeskForm, username, null).then(ticketId => {
        expect(ticketId).toBe(1234)
        done()
      })
    })
